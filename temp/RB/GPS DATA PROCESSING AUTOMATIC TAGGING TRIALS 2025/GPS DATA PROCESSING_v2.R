...
for (i in 1:dim(marking.df)[1]) {
  ...
  end.datetime <-marking.df$end.datetime.posix[i]
  ...
  #-----FIND EXISTING JASON FILES
  
  these.files<-grep(marking.df$GPS.ID[i],file.list,value = TRUE)
  
  f<-these.files[[1]]
  result <- range(unlist(lapply(these.files,function(f){
    out<-rjson::fromJSON(file = file.path(dat.path,f))
    range(unlist(out)[names(unlist(out))%in%"receivedAt"])
  })))
  
  temp <- gsub("T", " ", result)
  temp <- gsub("Z", "", temp)
  temp<-as.POSIXct(strptime(temp, "%Y-%m-%d %H:%M:%S")
                     , tz = "UTC")
  

  
  start.datetime<-as.POSIXct(ifelse(start.datetime<max(temp),max(temp),start.datetime), tz = "UTC")
  },silent=TRUE)
...
for (i in 1:dim(marking.df)[1]) {
  ...
  try({
    ...
    f<-these.files[[1]]
    json.all<-lapply(these.files, function(f)rjson::fromJSON(file=file.path(dat.path,f)))
    
    
    json.one<-toJSON(do.call(c,json.all))
    result<-rjson::fromJSON(json.one)
    
    
    new.label<-gsub("ardufox","GPS",paste(marking.df$GPS.ID[i],".json",sep=""))
    
    path <-
      file.path(dat.path,new.label)
    ...
    temp$datetime.posix<-as.POSIXct(temp$datetime.posix)
    
    #str(all.data$datetime.posix)
    
    
    time.xlim<-as.POSIXct(range(c(range(all.data$datetime.posix,na.rm=TRUE),current.posix)))
    ...
    try({
      ...
      #######################################################
      #     IDENTIFY AND REMOVE ERRANT POSITIONS (TOO QUICK MOVEMENTS)
      #######################################################
      ...
      gps.sf<-gps.sf%>%
        ...
        mutate(
          row.id=row_number(),
          duration.to.next = as.numeric(lead(datetime.posix) - datetime.posix),
          distance.to.next = as.numeric(sf::st_distance(
            geometry, 
            lead(geometry), 
            by_element = TRUE)),
          duration.to.previous = as.numeric(datetime.posix - lag(datetime.posix)),
          distance.to.previous = as.numeric(sf::st_distance(
            lag(geometry), 
            geometry, 
            by_element = TRUE))
        )
      
      gps.sf$speed.to.previous<-gps.sf$distance.to.previous/gps.sf$duration.to.previous
      gps.sf$speed.to.next<-gps.sf$distance.to.next/gps.sf$duration.to.next
      
      mapview(gps.sf,zcol="speed.to.previous")
      
      #--- NEED TO ADJUST THESE FOR DEER
      short.speed.threshold<-10 #(in m/sec; max speed about 36km/hour maintained over 30 second interval); 5*60*60/1000
      long.speed.threshold<-4 #(in m/sec; max speed about 36km/hour maintained over 100 meters); 4*60*60/1000
      
      row.id.toss<-gps.sf%>%
        st_drop_geometry()%>%
        #mutate(toss=(speed.to.previous>short.speed.threshold | speed.to.next>short.speed.threshold)   | ((speed.to.previous>long.speed.threshold & distance.to.previous>100 )  |   (speed.to.next>long.speed.threshold & distance.to.next>200)) )%>%filter(toss)%>%dplyr::select(row.id)%>%unlist()
        mutate(toss=(speed.to.previous>short.speed.threshold & speed.to.next>short.speed.threshold )   | (speed.to.next>short.speed.threshold & duration.to.previous>30*60 ) )%>%filter(toss)%>%dplyr::select(row.id)%>%unlist()
      
      temp<-gps.sf%>%mutate(to.toss=row.id%in%row.id.toss)
      
       track.sf<-temp%>%st_combine()%>%
        st_cast("LINESTRING")
      mapview(temp,zcol="to.toss")+mapview(track.sf)
      
      gps.sf<-gps.sf%>%filter(!row.id%in%row.id.toss)
      
      
      dat$gps.sf <- gps.sf
      dat$gps.df <- gps.sf%>%st_drop_geometry()
